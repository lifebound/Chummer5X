...
class EnhancedChummerXmlService {
  ...
  static ShadowrunCharacter? parseCharacterXml(String xmlContent) {
    try {
      ...
      String name = _getElementText(characterElement, 'name') ??
          _getElementText(characterElement, 'alias') ??
          'Unknown';
      if (name.isEmpty) {
      ...
      final publicAwareness =
          _getElementText(characterElement, 'publicawareness');
      final karma = _getElementText(characterElement, 'karma');
      final totalKarma = _getElementText(characterElement, 'totalkarma');
      final nuyen =
          int.tryParse(_getElementText(characterElement, 'nuyen') ?? '0') ?? 0;
      ...
      // Parse skills
      final skills =
          _parseSkills(characterElement, skillGroups, prioritySkills);
      ...
      final submersionGrades =
          _parseInitiationGrades(characterElement)[1] as List<SubmersionGrade>;

      // Parse gear
      final gear = _parseGear(characterElement);
      final gearLocations = _parseLocations(characterElement, "gearlocations",
          defaultGearLocationGuid, 'Selected Gear');

      // parse armor
      final allArmor = _parseArmor(characterElement);
      final armorLocations = _parseLocations(characterElement, "armorlocations",
          defaultArmorLocationGuid, 'Selected Armor');

      //parse vehicles
      final vehicles = _parseVehicles(characterElement);
      final vehicleLocations = _parseLocations(characterElement,
          "vehicleLocations", defaultVehicleLocationGuid, 'Selected Vehicle');

      // parse weapons
      final allWeapons = _parseWeapons(characterElement);
      final weaponLocations = _parseLocations(characterElement,
          "weaponLocations", defaultWeaponLocationGuid, 'Selected Weapon');

      //parse cyberware and bioware
      final allCyberware = _parseCyberware(characterElement);
      final cyberwareLocations = _parseLocations(characterElement,
          "cyberwareLocations", defaultCyberwareLocationGuid, 'Cyberware');
      cyberwareLocations.addAll(_parseLocations(characterElement,
          "biowareLocations", defaultBiowareLocationGuid, 'Bioware'));
      ...
      // Parse enabled flags
      final magEnabled =
          _getElementText(characterElement, 'magenabled')?.toLowerCase() ==
              'true';
      final resEnabled =
          _getElementText(characterElement, 'resenabled')?.toLowerCase() ==
              'true';
      final depEnabled =
          _getElementText(characterElement, 'depenabled')?.toLowerCase() ==
              'true';

      // Parse character type flags (for Shadowrun 5e: adept, magician, technomancer)
      final isAdept =
          _getElementText(characterElement, 'adept')?.toLowerCase() == 'true';
      final isMagician =
          _getElementText(characterElement, 'magician')?.toLowerCase() ==
              'true';
      final isTechnomancer =
          _getElementText(characterElement, 'technomancer')?.toLowerCase() ==
              'true';
      ...
      final karmaExpenseEntries = allExpenseEntries
          .where((entry) => entry.type == ExpenseType.karma)
          .toList();
      final nuyenExpenseEntries = allExpenseEntries
          .where((entry) => entry.type == ExpenseType.nuyen)
          .toList();
  ...
  static List<Attribute> _parseAttributes(XmlElement characterElement) {
    ...
    final attributesElement =
        characterElement.findElements('attributes').firstOrNull;
    ...
    if (attributesElement != null) {
      ...
      for (final attributeElement in attributeElements) {
        ...
        if (name != null && (name != 'Essence' && name != 'ESS')) {
          // Skip ESS from XML, it will be hard-coded in constructor
          // Parse all the fields from the XML
          final metatypeCategory =
              _getElementText(attributeElement, 'metatypecategory') ?? '';
          final totalValue = double.tryParse(
                  _getElementText(attributeElement, 'totalvalue') ?? '0') ??
              0.0;
          final metatypeMin = double.tryParse(
                  _getElementText(attributeElement, 'metatypemin') ?? '0') ??
              0.0;
          final metatypeMax = double.tryParse(
                  _getElementText(attributeElement, 'metatypemax') ?? '0') ??
              0.0;
          final metatypeAugMax = double.tryParse(
                  _getElementText(attributeElement, 'metatypeaugmax') ?? '0') ??
              0.0;
          final base = double.tryParse(
                  _getElementText(attributeElement, 'base') ?? '0') ??
              0.0;
          final karma = double.tryParse(
                  _getElementText(attributeElement, 'karma') ?? '0') ??
              0.0;
  ...
  static List<SkillGroup> _parseSkillGroups(XmlElement characterElement) {
    ...
    final skillsElement =
        characterElement.findElements('newskills').firstOrNull;
  ...
  static List<Skill> _parseSkills(XmlElement characterElement,
      List<SkillGroup> skillGroups, Set<String> prioritySkills) {
    final skills = <Skill>[];
    ...
    if (skillsElement != null) {
      final skillsListElement =
          skillsElement.findElements('skills').firstOrNull;
          ...
          if (name != null) {
            ...
            if (skillGroupName.isNotEmpty) {
              final matchingGroup = skillGroups.firstWhere(
                ...
                orElse: () =>
                    const SkillGroup(name: '', base: null, karma: null),
              );
              if (matchingGroup.name.isNotEmpty) {
                skillGroupTotal = SkillGroupMap.calculateGroupTotal(
                    matchingGroup.base, matchingGroup.karma);
              }
            }

            debugPrint(
                'Parsing skill: $name, Karma: $karma, Base: $base, Group: $skillGroupName, GroupTotal: $skillGroupTotal');
            ...
            if (specsElement != null) {
              ...
              if (specElements.isNotEmpty) {
                ...
                for (final specElement in specElements) {
                  ...
                  final expertise =
                      _getElementText(specElement, 'expertise') ?? 'False';
  ...
  static Set<String> _parsePrioritySkills(XmlElement characterElement) {
    ...
    final prioritySkillsElements =
        characterElement.findElements('priorityskills');
    debugPrint(
        'Found ${prioritySkillsElements.length} priority skills elements');
    ...
    for (final prioritySkillsElement in prioritySkillsElements) {
      ...
      for (final prioritySkillElement
          in prioritySkillsElement.findElements('priorityskill')) {
  ...
  static void _checkBrokenSkillGroups(
      List<SkillGroup> skillGroups, List<Skill> skills) {
    for (final group in skillGroups) {
      final groupTotal =
          SkillGroupMap.calculateGroupTotal(group.base, group.karma);
      if (groupTotal > 0) {
        final isBroken =
            SkillGroupMap.isGroupBroken(group.name, skills, groupTotal);
        if (isBroken) {
          debugPrint('WARNING: Skill group "${group.name}" is broken!');
        } else {
          debugPrint(
              'Skill group "${group.name}" is intact (total: $groupTotal)');
        }
      }
    }
  }
  ...
  static List<Spirit> _parseSpirits(XmlElement characterElement) {
    ...
    if (spiritsElement != null) {
      for (final spiritElement in spiritsElement.findElements('spirit')) {
        ...
        final services =
            int.tryParse(_getElementText(spiritElement, 'services') ?? '0') ??
                0;
        final bound =
            _getElementText(spiritElement, 'bound')?.toLowerCase() == 'true';
        final fettered =
            _getElementText(spiritElement, 'fettered')?.toLowerCase() == 'true';
        final force =
            int.tryParse(_getElementText(spiritElement, 'force') ?? '') ?? 0;
        final crittername = _getElementText(spiritElement, 'crittername');

        if (name != null && type != null && type == 'Spirit') {
          final critter = CritterFactory.generateSpirit(
              name, force, services, bound, fettered,
              nameOverride: crittername);
    ...
    if (spiritsElement != null) {
      for (final spiritElement in spiritsElement.findElements('spirit')) {
        ...
        if (name != null && type != null && type == 'Sprite') {
          final critter = CritterFactory.generateSprite(
              name, force, services, bound, fettered,
              nameOverride: crittername);
    ...
    for (final element in martialArtsElements) {
      ...
      for (final martialArtElement in martialArtElements) {
        try {
          ...
          for (final child
              in martialArtElement.children.whereType<XmlElement>()) {
  ...
  static Map<String, dynamic> _parseMartialArtTechniques(
      XmlElement techniquesElement) {
    final techniquesList = <Map<String, dynamic>>[];

    final techniqueElements =
        techniquesElement.findElements('martialarttechnique');
  ...
  static List<Gear> _parseGear(XmlElement characterElement) {
    ...
    final allGear = /*<Gear>[];*/ characterElement.parseList(
  ...
  static List<Vehicle> _parseVehicles(XmlElement characterElement) {
    ...
    final vehiclesElement =
        characterElement.findElements('vehicles').firstOrNull;
  ...
  static Map<String, Location> _parseLocations(XmlElement characterElement,
      String locationType, String defaultLocationGuid, String defaultName) {
    debugPrint('Parsing locations of type: $locationType');
    final locations = <String, Location>{};
    final locationsElement =
        characterElement.findElements(locationType).firstOrNull;
    ...
    if (locationsElement != null) {
      for (final locationElement in locationsElement.findElements('location')) {
        ...
        final sortOrder = int.tryParse(
                _getElementText(locationElement, 'sortorder') ?? '0') ??
            0;
  ...
  static ConditionMonitor _parseConditionMonitor(
      XmlElement characterElement, Map<String, dynamic> calculatedValues) {
  ...
  static Map<String, dynamic> _parseCalculatedValues(
      XmlElement characterElement) {
    debugPrint('Parsing calculated values...');
    final calculatedValuesElement =
        characterElement.findElements('calculatedvalues').firstOrNull;
    if (calculatedValuesElement == null) return {};

    final calculatedValues = <String, dynamic>{};
    for (final element
        in calculatedValuesElement.children.whereType<XmlElement>()) {
  ...
  static List<Quality> _parseQualities(XmlElement characterElement) {
    ...
    final qualitiesElement =
        characterElement.findElements('qualities').firstOrNull;
    ...
    if (qualitiesElement != null) {
      for (final qualityElement in qualitiesElement.findElements('quality')) {
        ...
        final karmaCost =
            int.tryParse(_getElementText(qualityElement, 'bp') ?? '0') ?? 0;
        final qualityTypeStr =
            _getElementText(qualityElement, 'qualitytype')?.toLowerCase() ?? '';
        final qualityType = qualityTypeStr == 'positive'
  ...
  static List<List<InitiationGrade>> _parseInitiationGrades(
      XmlElement characterElement) {
    ...
    final gradesElement =
        characterElement.findElements('initiationgrades').firstOrNull;

    if (gradesElement != null) {
      for (final gradeElement
          in gradesElement.findElements('initiationgrade')) {
        debugPrint('Parsing grade element: $gradeElement');
        bool isResEnabled = _getElementText(gradeElement, 'res') == 'True';

        final grade =
            int.tryParse(_getElementText(gradeElement, 'grade') ?? '0') ?? 0;
        final ordeal =
            _getElementText(gradeElement, 'ordeal')?.toLowerCase() == 'true';
        final group =
            _getElementText(gradeElement, "group")?.toLowerCase() == 'true';
        final schooling =
            _getElementText(gradeElement, 'schooling')?.toLowerCase() == 'true';
      ...
      for (Metamagic metamagic in metamagics) {
        ...
        if (metamagic.improvementSource.toLowerCase() == 'echo') {
          ...
          try {
            ...
            final matchingGrade = submersionGrades
                .where((grade) => grade.grade == metamagic.grade);
          ...
          try {
            ...
            final matchingGrade = initiationGrades
                .where((grade) => grade.grade == metamagic.grade);
            matchingGrade.firstOrNull?.metamagics.add(metamagic);
          } catch (e) {
    ...
    debugPrint(
        'Total initiation grades: ${initiationGrades.length}, Total submersion grades: ${submersionGrades.length}');
  ...
  static List<Metamagic> _parseMetamagics(XmlElement characterElement) {
    ...
    final enchantmentsElement =
        characterElement.findElements('spells').firstOrNull;
    debugPrint('Parsing enchantments...');
    if (enchantmentsElement != null) {
      debugPrint('Found enchantments element, processing spells...');
      for (final enchantmentElement
          in enchantmentsElement.findElements('spell')) {
        final category = _getElementText(enchantmentElement, 'category');
        debugPrint(
            'Processing enchantment: ${_getElementText(enchantmentElement, 'name')} with category $category');
        if (category == null &&
            category?.toLowerCase() != 'enchantments' &&
            category?.toLowerCase() != 'rituals') {
          debugPrint(
              'Skipping non-enchantment spell: ${_getElementText(enchantmentElement, 'name')}');
          continue;
        }
        ...
        final improvementSource =
            _getElementText(enchantmentElement, 'improvementsource') ?? '';
        final grade = _getElementText(enchantmentElement, 'grade');
        debugPrint(
            'name: $name, source: $source, page: $page, improvement source: $improvementSource, grade: $grade');
        if (name != null) {
    ...
    final enhancementsElement =
        characterElement.findElements('enhancements').firstOrNull;
    if (enhancementsElement != null) {
      for (final enhancementElement
          in enhancementsElement.findElements('enhancement')) {
        ...
        final improvementSource =
            _getElementText(enhancementElement, 'improvementsource') ?? '';
  ...
  static List<Metamagic> _parseMetamagicType(
      XmlElement characterElement, String type) {
    debugPrint('Parsing metamagics of type: $type');
    final metamagics = <Metamagic>[];
    final metamagicsElement =
        characterElement.findElements('${type}s').firstOrNull;
    ...
    if (metamagicsElement != null) {
      for (final metamagicElement in metamagicsElement.findElements(type)) {
        ...
        final improvementSource =
            _getElementText(metamagicElement, 'improvementsource') ?? '';
  ...
  static GameNotes? _parseGameNotes(XmlElement characterElement) {
    ...
    final gameNotesElement =
        characterElement.findElements('gamenotes').firstOrNull;
  ...
  static List<ExpenseEntry> _parseExpenseEntries(XmlElement characterElement) {
    ...
    final expenseEntriesElement =
        characterElement.findElements('expenses').firstOrNull;

    if (expenseEntriesElement != null) {
      for (final entryElement
          in expenseEntriesElement.findElements('expense')) {
  ...
  static Mugshot? _parseMugshot(XmlElement characterElement) {
    ...
    try {
      final mugshotsElement =
          characterElement.findElements('mugshots').firstOrNull;
      ...
      debugPrint(
          'Found mugshot base64 data: ${base64Data.substring(0, 20)}...'); // Log first 20 chars for brevity
  ...
  static List<Cyberware> _parseCyberware(XmlElement characterElement) {
    ...
    final cyberwareElements =
        characterElement.findElements('cyberwares').firstOrNull;
    if (cyberwareElements == null) {
      debugPrint('No cyberware elements found.');
      return [];
    }
    final individualCyberwareElements =
        cyberwareElements.findElements('cyberware');
    debugPrint(
        'Parsed ${individualCyberwareElements.length} cyberware entries.');
    return individualCyberwareElements
        .map((e) => Cyberware.fromXml(e))
        .toList();
  }

  static List<Contact> _parseContacts(XmlElement characterElement) {
    debugPrint('Parsing contacts...');
    final contactsElements =
        characterElement.findElements('contacts').firstOrNull;
